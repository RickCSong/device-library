...
export default Ember.Component.extend({
  ...
  sortedContent: (function() {
    if (this.get('sorted')) {
      ...
      return this.get('content').sortBy(optionLabelPath);
    } else {
...
*/
    ...
    options.query = function(query) {
      ...
      if (self.get('_typeaheadMode')) {
        ...
        var filteredContent = self.get("sortedContent").reduce(function(results, item) {
    ...
    options.initSelection = function(element, callback) {
      ...
      Ember.assert("select2#initSelection can not map string values to full objects " +
        "in typeahead mode. Please open a github issue if you have questions to this.", !self.get('_typeaheadMode'));
